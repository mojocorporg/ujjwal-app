  ...
  Future<void> getOtp() async {
    ...
    final PhoneVerificationCompleted verificationCompleted =
      ...
      UserPreferences().saveData(
          UserPreferences.SHARED_USER_NUMBER_TWO, phoneController.text);
      UserPreferences()
          .saveData(UserPreferences.SHARED_COUNTRY_CODE, countryCode);
    ...
    await _firebaseMessaging.getToken().then((String token) {
      ...
      LoginRepo().loginUser(params).then((value) {
        if (value != null) {
          if (value.status) {
            UserPreferences()
                .saveData(UserPreferences.SHARED_USER_TOKEN, value.token);
            UserPreferences().saveData(
                UserPreferences.SHARED_USER_ID, value.userId.toString());
            UserPreferences().saveData(UserPreferences.SHARED_USER_REFERRAL,
